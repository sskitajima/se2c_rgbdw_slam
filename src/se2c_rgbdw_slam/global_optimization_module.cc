...
#include "se2c_rgbdw_slam/match/fuse.h"
...
global_optimization_module::~global_optimization_module() {
    ...
    if(thread_for_octomap_reconstruction_)
    {
        thread_for_octomap_reconstruction_->join();
        // thread_for_octomap_reconstruction_.reset(nullptr);
    }
...
void global_optimization_module::set_octomap_mapping_module(module::octomap_mapping* octomap_mapping) {
    octomap_mapping_ = octomap_mapping;
}

void global_optimization_module::set_se2c_wrappers(std::shared_ptr<optimize::g2o::se2c::plane_constraint_wrapper>& plane_wrapper, std::shared_ptr<optimize::g2o::se2c::odometry_constraint_wrapper>& odom_wrapper)
{
    graph_optimizer_->set_se2c_wrappers(plane_wrapper, odom_wrapper);
    loop_bundle_adjuster_->set_se2c_wrappers(plane_wrapper, odom_wrapper);
}
...
void global_optimization_module::correct_loop() {
    ...
    // Proposed
    thread_for_octomap_reconstruction_ = std::unique_ptr<std::thread>(new std::thread(&module::octomap_mapping::rebuild_octree_for_all_keyfrms, octomap_mapping_));
