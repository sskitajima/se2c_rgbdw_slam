...
#include "se2c_rgbdw_slam/data/graph_node.h"
...
public:
    ...
    /**
     * Proposed for Octomap Mapping
     */
     void set_depthmap(const cv::Mat& depthmap);
    ...
    //! Proposed
    void set_referenced_flag(const bool flag){is_referenced_keyfrm_=flag;}
    bool is_referenced_keyfrm() const {return is_referenced_keyfrm_;}
    void set_odom_from_ref_kf(const std::pair<keyframe*, std::pair<Mat44_t, Mat66_t>>& odom_from_ref_kf);
    ...
    //! Proposed
    std::shared_ptr<std::pair<keyframe*, std::pair<Mat44_t, Mat66_t>>> odom_from_ref_kf_ = nullptr;
    std::shared_ptr<std::pair<keyframe*, std::pair<Mat44_t, Mat66_t>>> odom_to_ref_kf_ = nullptr;
...
private:
    ...
    //! Proposed
    bool is_referenced_keyfrm_ = false;
    mutable std::mutex mtx_depthmap_;
    cv::Mat depthmap_;
