...
#include "se2c_rgbdw_slam/data/common.h"
...
#include "se2c_rgbdw_slam/feature/orb_params.h"
...
void keyframe::set_depthmap(const cv::Mat& depthmap)
{
    std::lock_guard<std::mutex> lock(mtx_depthmap_);
    depthmap_ = depthmap.clone();
}

cv::Mat keyframe::get_depthmap() const
{
    std::lock_guard<std::mutex> lock(mtx_depthmap_);
    return depthmap_;
}
...
void keyframe::prepare_for_erasing() {
    ...
    // std::cout << "[Keyframe::prepare_for_erasing()] id_: " << id_ << std::endl;

    //! Proposed
    if(odom_to_ref_kf_ != nullptr)
    {
        Mat44_t odom_last_to_this = odom_from_ref_kf_->second.first;
        Mat44_t odom_this_to_next = odom_to_ref_kf_->second.first;
        Mat66_t cov_last_to_this = odom_from_ref_kf_->second.second;
        Mat66_t cov_this_to_next = odom_to_ref_kf_->second.second;

        Mat44_t odom_last_to_next = odom_last_to_this * odom_this_to_next;
        Mat66_t cov_last_to_next = cov_last_to_this + cov_this_to_next;     //! TODO: apply uncertainty propagation model
        
        keyframe* previous_keyfrm = odom_from_ref_kf_->first;
        keyframe* next_keyfrm = odom_to_ref_kf_->first;

        std::pair<data::keyframe*, std::pair<Mat44_t, Mat66_t>> odom_to_next 
                = std::make_pair(next_keyfrm, std::make_pair(odom_last_to_next, cov_last_to_next));

        std::pair<data::keyframe*, std::pair<Mat44_t, Mat66_t>> odom_from_previous
                = std::make_pair(previous_keyfrm, std::make_pair(odom_last_to_next, cov_last_to_next));
        

        previous_keyfrm->odom_to_ref_kf_ = std::make_shared<std::pair<data::keyframe*, std::pair<Mat44_t, Mat66_t>>>(odom_to_next);
        next_keyfrm->odom_from_ref_kf_ = std::make_shared<std::pair<data::keyframe*, std::pair<Mat44_t, Mat66_t>>>(odom_from_previous);
    }
    else
    {
        keyframe* previous_keyfrm = odom_from_ref_kf_->first;
        previous_keyfrm->odom_to_ref_kf_ = nullptr;
    }
}
