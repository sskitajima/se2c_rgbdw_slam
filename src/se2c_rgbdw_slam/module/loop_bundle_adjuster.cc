...
#include "se2c_rgbdw_slam/module/loop_bundle_adjuster.h"
...
void loop_bundle_adjuster::set_se2c_wrappers(std::shared_ptr<optimize::g2o::se2c::plane_constraint_wrapper>& plane_wrapper, std::shared_ptr<optimize::g2o::se2c::odometry_constraint_wrapper>& odom_wrapper)
{
...
void loop_bundle_adjuster::optimize(const unsigned int identifier) {
    ...
    auto global_bundle_adjuster = optimize::global_bundle_adjuster(map_db_, num_iter_, false);
    global_bundle_adjuster.set_se2c_wrappers(plane_constraint_wrapper_, odometry_constraint_wrapper_);
...

    {
        ...
        for (auto lm : landmarks) {
            ...
            if (lm->loop_BA_identifier_ == identifier) {
                ...
                // std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
                // std::cout << "[loop_bundle_adjuster::optimize()] ref_keyfrm->loop_BA_identifier_ " << ref_keyfrm->loop_BA_identifier_ 
                // << "  identifier " << identifier << " lm->loop_BA_identifier_ " << lm->loop_BA_identifier_ << " KF id: " << ref_keyfrm->id_ <<  std::endl;
                
                // LMを保持しているKFは loop BA によって最適化されていることを確認する
                // ループとじ込み中に追加されたKFは対象にしない（バグ?）
                if(ref_keyfrm->id_ > identifier) continue;
