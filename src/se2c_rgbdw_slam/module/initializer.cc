...
#include "se2c_rgbdw_slam/initialize/bearing_vector.h"
#include "se2c_rgbdw_slam/initialize/perspective.h"
...
initializer::initializer(const camera::setup_type_t setup_type,
    ...
    : setup_type_(setup_type), map_db_(map_db), bow_db_(bow_db),
      ...
      enable_plane_estimate_(yaml_node["Initializer.enable_plane_estimate"].as<bool>(false))
{
    plane_estimator_ = new plane_estimator(yaml_node);
...
initializer::~initializer() {
    ...
    delete plane_estimator_;
    plane_estimator_ = nullptr;
}
...
void initializer::set_coord_transformer(std::shared_ptr<util::coordinate_transformer>& coord_transformer)
{
    coord_transformer_ = coord_transformer;
}

bool initializer::initialize(data::frame& curr_frm, const Mat44_t& ext_param_cr) {
    switch (setup_type_) {
...
void initializer::set_depth_img(const cv::Mat& depth_img)
{
...
bool initializer::create_map_for_stereo(data::frame& curr_frm) {
    ...
    //! Proposed
    if(enable_plane_estimate_)
    {
        assert(!depth_img_.empty());
        plane_estimator_->set_extrinsic_rc(coord_transformer_->get_rc());
        Mat44_t est_transform_rc = plane_estimator_->estimate(depth_img_);
        coord_transformer_->set_rc(est_transform_rc);
    }

    // create an initial keyframe
    // curr_frm.set_cam_pose(Mat44_t::Identity());
    curr_frm.set_cam_pose(coord_transformer_->get_cr());
    auto curr_keyfrm = new data::keyframe(curr_frm, map_db_, bow_db_);
