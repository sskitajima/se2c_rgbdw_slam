...
#include "se2c_rgbdw_slam/match/robust.h"
...
void frame_tracker::set_se2c_wrappers(std::shared_ptr<optimize::g2o::se2c::plane_constraint_wrapper>& plane_wrapper, std::shared_ptr<optimize::g2o::se2c::odometry_constraint_wrapper>& odom_wrapper)
{
...
bool frame_tracker::motion_based_track(data::frame& curr_frm, const data::frame& last_frm, const Mat44_t& velocity) const {
    ...
    spdlog::debug("motion_based_track num_matches before optimization {}", num_matches);
    ...
    const auto track_time = std::chrono::duration_cast<std::chrono::duration<double>>(finish_time-start_time).count();
    ...
    if (num_valid_matches < num_matches_thr_) {
        ...
        spdlog::debug("motion_based_track tracking time: " + std::to_string(track_time));
        spdlog::debug("motion_based_track num_matches : " + std::to_string(num_valid_matches));
...
bool frame_tracker::bow_match_based_track(data::frame& curr_frm, const data::frame& last_frm, data::keyframe* ref_keyfrm) const {
    ...
    spdlog::debug("bow_based_track num_matches before optimization {}", num_matches);
    ...
    if (num_valid_matches < num_matches_thr_) {
        ...
        spdlog::debug("bow_based_track tracking time: " + std::to_string(track_time));
        spdlog::debug("bow_based_track num_matches : " + std::to_string(num_valid_matches));
...
bool frame_tracker::robust_match_based_track(data::frame& curr_frm, const data::frame& last_frm, data::keyframe* ref_keyfrm) const {
    ...
    if (num_valid_matches < num_matches_thr_) {
        ...
        spdlog::debug("robust_track tracking time: " + std::to_string(track_time));
        spdlog::debug("robust_track num_matches : " + std::to_string(num_valid_matches));
